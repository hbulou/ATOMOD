...
\begin{itemize}
	\item 1 - Open a PowerShell (Fig.\ref{fig:install2}(1-2-3))
	\item 2 - Navigate to the directory of your choice (e.g., \verb+cd ~/Desktop+ or \verb+cd ~/src+)(Fig.\ref{fig:install2}(4)).
	
	\verb+cd C:\Users\herve\Desktop\+
	
	
	
	\item 3 - Download the installation script \verb+install.py+.
	
	{\small
	\verb+wget https://github.com/hbulou/ATOMOD/releases/download/v0.1/install.py-OutFile install.py+
}
	\item 4 - Run the installation (Fig.\ref{fig:install2}(5)). NOTE: You must have at least 7 GB of available disk space. 
	
	\verb+python.exe install.py+
	
	\item 5 - Once the installation is complete, go to the ATOMOD directory (Fig.\ref{fig:install2}(6)).
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\linewidth]{install2}
	\caption{Screenshots of the various stages of installing ATOMOD on Windows.}
	\label{fig:install2}
\end{figure}
...
% Mise à jour
% ============================================================================
\section*{ATOMOD Update}

%Pour mettre à jour ATOMOD, il suffit d'exéécuter le script \verb+update_ATOMOD.py+ :

To update ATOMOS, simply run the \verb+update_ATOMOD.py+ script  (Fig.\ref{fig:install2}(8)):

\verb+python3 update_ATOMOD.py+
...
To run the tutorials, the simplest method is to use the \verb+run_tuto_ATOMOD.py+ file located in the \verb+ATOMOD+ directory ( (Fig.\ref{fig:install2}(7)):

\verb+ python3 run_tuto_ATOMOD.py+

Note regarding \verb+03-tuto-EXAFS.ipynb+: the tutorial was developed for use with \verb+FEFF+ installed outside the \verb+ATOMOD+ distribution; therefore, this tutorial will not work if you do not have \verb+FEFF+ installed on your computer.

%Remarque concernant  \verb+03-tuto-EXAFS.ipynb+ : le tutoriel a été développé pour être utilisé avec \verb+FEFF+ installé  en dehors de la distribution \verb+ATOMOD+ aussi ce tutoriel ne fonctionnera pas si vous n'avez pas \verb+FEFF+ installé sur votre ordinateur.
